...
$MESSAGE_RENEW_PORT_NUMBER = "__RPN__"
  ...
  while ($true) {
    if ($tcpConnection.Connected) {
       ...
       $RequestString = $RequestString.Substring(0,$RequestString.Length-1)
        ...
        if ($RequestString.StartsWith($MESSAGE_RENEW_PORT_NUMBER)) {
            $serverPortString = $RequestString.Replace($MESSAGE_RENEW_PORT_NUMBER, '')
            $serverPort = [int]$serverPortString
            Write-Host "Received 'renew port' message. " + $serverPortString + "]"
